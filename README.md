...
The use of GitHub Actions is suitable for development and drafting but not for creating the final distribution. Final distribution needs to be performed offline on one's computer so that the target directory of the artefacts and the target directory of the hub document are the same directory. This triggers the hub document creation also to perform integrity checks regarding missing artefact files referenced from the hub document XML. Alternatively, this hub process can be run offline independently without first having run the artefacts process, but then a complete distribution package is not created.

The artefacts-production process is performed first, either locally or on GitHub, followed by this hub-production process. When performed locally, the outputs are directed to some result directory. When performed on GitHub, the results need to be downloaded, unzipped, and then renamed to have the appropriate label. For example, the `artefacts-UBL-2.3-github-20201118-200351z.zip` file downloaded from GitHub unzips to create the `artefacts-UBL-2.3-csd04-20201118-200351z.zip` file. This, in turn, is unzipped to create the `artefacts-UBL-2.3-csd04-20201118-200351z` subdirectory. This subdirectory is renamed replacing the timestamp with the label, in this example to be `artefacts-UBL-2.3-csd04-test`. This renamed directory is placed into the target results directory ready to be referenced by the hub publishing environment, which must be performed locally and not on GitHub in order to produce the final results.
