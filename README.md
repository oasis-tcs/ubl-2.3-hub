...
1. In the downloaded ZIP look in the `archive-only-not-in-final-distribution/` subdirectory for a `new-entities/` directory with any new entity files that were created by your changes to the schema and party XML files; if there are any new entity file, you must replace your local copies with those new entities in order for the validation in your XML editor to match the validation results on the server; if the `new-entities` directory does not exist, then there is no need to update your local entity files
